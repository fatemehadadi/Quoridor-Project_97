...
class RegisterPage(Frame):
    def __init__(self, parent, controller):
        ...
        label = Label(self, text="Fill In The Data", font=controller.title_font)
        label.pack(side="top", fill="x", pady=10)
        label = Label(self, text="Name", font=controller.title_font).pack()
        label = Label(self, text="Username", font=controller.title_font).pack()
        label = Label(self, text="password", font=controller.title_font).pack()
        label = Label(self, text="confirme password", font=controller.title_font).pack()

        button1 = tk.Button(self, text="Back",
                            command=lambda: self.controller.show_frame("StartPage"), bg="red")
        button1.pack()
        button = Button(self, text="Register",
                        command=lambda: self.is_valid_person())
        button.pack()
